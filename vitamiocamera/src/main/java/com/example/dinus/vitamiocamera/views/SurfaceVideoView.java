...
public class SurfaceVideoView extends SurfaceView implements Callback {
	...
	/** 定时暂停 */
	private static final int HANDLER_MESSAGE_PAUSE = 0;
	/** 定时循环 */
    ...
    public void setOnPlayProgressListener(OnPlayProgressListener mOnPlayProgressListener) {
        this.mOnPlayProgressListener = mOnPlayProgressListener;
    }
    ...
    public interface OnPlayProgressListener{
        void onProgressRate(float progressRate);
    }
	...
	public void start() {
		...
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
				if (!isPlaying()) {
                    mMediaPlayer.start();
					cancelPalyProgressTimer();
                    addPlayProgressTimer();
                }
	...
	public void seekTo(float seekRate) {
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
				if (seekRate < 0)
					seekRate = 0;
				mMediaPlayer.seekTo((int) (seekRate * mMediaPlayer.getDuration()));
			} catch (IllegalStateException e) {
				Logger.e(e);
			} catch (Exception e) {
				Logger.e(e);
	...
	private void addPlayProgressTimer(){
		updateProgressTimer = new Timer();
		updateProgressTimer.scheduleAtFixedRate(new TimerTask() {
			@Override
			public void run() {
				if (mMediaPlayer != null && mMediaPlayer.isPlaying()) {
                    float progress = mMediaPlayer.getCurrentPosition() * 1.0f / mMediaPlayer.getDuration();
                    if (mOnCompletionListener != null) {
                        mOnPlayProgressListener.onProgressRate(progress);
                    }
				}
			}
		}, 0, 50);
    ...
    private void cancelPalyProgressTimer(){
	...
	public void pauseDelayed(int delayMillis) {
		if (mVideoHandler.hasMessages(HANDLER_MESSAGE_PAUSE))
			mVideoHandler.removeMessages(HANDLER_MESSAGE_PAUSE);
		mVideoHandler.sendEmptyMessageDelayed(HANDLER_MESSAGE_PAUSE, delayMillis);
	}
